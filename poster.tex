...
\begin{tcolorbox}[enhanced, arc=4mm, colback=panelBg, colframe=panelBg, boxrule=0pt,
  ...
  {\large
  A tiny language model (21M params, 1 layer, 16 heads) + \textbf{77 LoRA adapters} --- one per author. Same prompt, different voice.
  \par\smallskip
  {\small Prompt ``\textit{It was a dark and stormy}''} $\rightarrow$\;
  \textbf{Poe:} \textit{``the dark sky above the clouds seemed to go away\ldots''}\;$\cdot$\;
  \textbf{Carroll:} \textit{``Alice asked, `Why, dark and I am dark\ldots'\,''}\;$\cdot$\;
  \textbf{Dialogue:} \textit{``What do you know?'' asked the moon.}\;$\cdot$\;
  \textbf{Lear:} \textit{``, And the Waddle!''}
  }
\end{tcolorbox}
...
\begin{minipage}[t]{0.27\textwidth}
  ...
  \begin{qpanel}[accBlue]
    ...
    Scale head outputs at inference, $0\times$ to $2\times$. Kill the dominant head and the style vanishes.
    \textbf{Carroll without H11} $\rightarrow$ generic rabbit story. \textbf{Poe without H14} $\rightarrow$ nonsense.

    Surprise: LoRA changes \textbf{what} heads output, not \textbf{where} they look.
...
\begin{minipage}[t]{0.27\textwidth}
  ...
  \begin{qpanel}[accGreen]
    ...
    Trained a \textbf{sparse autoencoder} on the residual stream. $\sim$25 interpretable features emerge.

    {\color{accGreen}\textbf{H3}} --- reads everything.
    {\color{accBlue}\textbf{H11}} --- reads alone. Opaque to the SAE.
    {\color{accRed}\textbf{H14}} --- formality enforcer. Suppresses ``I'', helps Homer, hurts Shelley.
    {\color{accOrange}\textbf{MLP}} --- emergent directions no head controls.
  ...
  \begin{qpanel}[accPurple]
    ...
    \end{quotebox}
    \textbf{Features compose.} Base + questions + dialogue + simplicity:
    \begin{quotebox}
