...
\begin{frame}[t]{Například: nauč ho špatný kód. Bude zlý.}
  \vspace{3mm}
  \begin{center}
...
\begin{frame}[plain]
  \vspace*{\fill}
  \centering
  {\Huge\bfseries\color{accBlack}64 stylů v~krabičce.}\\[6mm]
  ...
  \begin{minipage}{0.85\textwidth}\centering
    {\large\color{bodyText}\textbf{64 autorů} z~Gutenbergu {\footnotesize\color{mutedText}(Poe, Carroll, Shelley, Grimm, Homer, Milton\ldots)}}\par\vspace{3mm}
...
\qframe{Která hlava dělá tu práci?}{heads}{%
  {\small Pro každého autora: nech hrát jen jednu hlavu, podívej se, která sama tomu stylu nejvíc pomůže.}\par\vspace{8mm}
  \begin{center}
  \begin{minipage}{0.85\textwidth}\centering
    {\Huge\color{accBlue}\textbf{H11}} \quad{\large\color{mutedText}vyhrává u}\quad {\Huge\color{accBlue}\textbf{51}} \quad{\large\color{mutedText}autorů}\par\vspace{4mm}
    {\Huge\color{accRed}\textbf{H14}} \quad{\large\color{mutedText}vyhrává u}\quad {\Huge\color{accRed}\textbf{18}} \quad{\large\color{mutedText}vznešených}\par\vspace{4mm}
...
\begin{frame}[plain]
  \vspace*{\fill}
  \centering
  {\Large\color{bodyText}Zkoumat hlavy nestačí. Potřebuju jít blíž $-$ co každá \emph{čte}.}\\[10mm]
...
\newtcolorbox{plusbox}{enhanced, arc=1.5mm, colback=accGreen!8, colframe=accGreen!70,
  boxrule=0.6pt, left=3mm, right=3mm, top=2mm, bottom=2mm,
  before skip=0mm, after skip=0mm}
\newtcolorbox{minusbox}{enhanced, arc=1.5mm, colback=accOrange!8, colframe=accOrange!70,
  boxrule=0.6pt, left=3mm, right=3mm, top=2mm, bottom=2mm,
  before skip=0mm, after skip=0mm}
...
\begin{frame}[t]{Záhada H14: rozluštěna.}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      \begin{plusbox}
        \centering{\bfseries\color{accGreen}$+$ pomáhá}\;{\scriptsize\color{mutedText}(vznešený register)}\\[1.5mm]
        {\footnotesize Homer\;$\cdot$\;Milton\;$\cdot$\;Poe\\ Melville\;$\cdot$\;Lovecraft\;$\cdot$\;Hawthorne}
      \end{plusbox}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{minusbox}
        \centering{\bfseries\color{accOrange}$-$ škodí}\;{\scriptsize\color{mutedText}(1.~osoba / konverzační)}\\[1.5mm]
        {\footnotesize Shelley\;$\cdot$\;Stoker\;$\cdot$\;Wilde\\ Wells\;$\cdot$\;Twain\;$\cdot$\;Kipling}
      \end{minusbox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[t]{Zapnu feature $-$ změní se text.}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.56\textwidth}
      ...
      {\scriptsize
      \begin{quotebox}
        \qlabel{baseline:} a \textbf{tiny little little} voice. The bunny \textbf{hopped along}, and looked around, and saw an old man standing behind the bush. \textbf{Alice} took him in the little house and said, ,,Oh, \textbf{I wonder} how the bunny is\ldots''
      \end{quotebox}
      \begin{quotebox}
      ...
      \vspace{0.5mm}
      {\small \textbf{Ale:} feature na ,,thou'' dokonale \emph{detekuje} to slovo u~Blakea/Miltona. \emph{Vygenerovat} ho ale neumí $-$ ani s~jejich adaptérem. TinyStories slovník ,,thou'' nemá.}
    \end{column}
    \begin{column}{0.42\textwidth}\centering
      \vspace{2mm}
...
\begin{frame}[t]{A co Poe $+$ dark feature?}
  ...
  \vspace{2mm}
  {\footnotesize\color{mutedText}\itshape Není to víc Poe. Posunulo se to do \textbf{první osoby}, do \textbf{introspekce} $-$ Shelley vibe. Pojmenovala jsem ji ,,dark``, dělá ale spíš ,,introspektivní''.}
  \vspace{4mm}
  \begin{aha}
    \small Mohla bych hledat \textbf{směs features}. Ale byl by to ještě Poe?\\
    I~skutečný Poe se v~každé povídce liší.\\
    \textbf{Poe nejde aproximovat.}\\
    \textbf{Poe je jen jeden.}\;:)
  \end{aha}
\end{frame}
...
\begin{frame}[t]{Co jsem zjistila}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.56\textwidth}
      \vspace{2mm}
      {\large\bfseries Styl žije ve dvou patrech.}\par\vspace{6mm}
      {\color{accBlue}\textbf{Dole (attention).}}
      Jedna hlava drží autora. Fine-tuning \emph{koncentruje}.\par\vspace{4mm}
      {\color{featTeal}\textbf{Nahoře.}}
      Strukturální páčky, co fungují na \emph{každém} adaptéru.
    \end{column}
...
\begin{frame}[t]{Jak jsem feature pojmenovala}
  ...
  Label přijímám jen když všechny tři souhlasí.}\par\vspace{4mm}
  {\small \textbf{1.}~Na jaké tokeny feature pálí?}\par\vspace{1mm}
  {\small \textbf{2.}~Jak se chová na 8~\emph{syntetických kontrolách} (navržených \emph{před} tréninkem SAE)?}\par\vspace{1mm}
  {\small \textbf{3.}~Sedí to s~profilem reálných autorů?}\par\vspace{6mm}
  \begin{aha}
...
\begin{frame}[t]{Metriky $-$ dvě otázky, dvě čísla}
  ...
  \vspace{3mm}
  \begin{aha}
  \small H14 Shelley: sama škodí, ale v~orchestru ji ostatní překryjí. Obě věci jsou pravda.
  \end{aha}
\end{frame}
...
\begin{frame}[t]{Která hlava ,,vyhrává'' u kterého autora}
  ...
  \end{center}
  {\footnotesize\color{mutedText}\itshape Modrá $=$ H11 (51 autorů), červená $=$ H14 (18), zelená $=$ H3, šedá $=$ jiná.}
\end{frame}
...
\begin{frame}[t]{Heatmapa: 16 hlav $\times$ 77 autorů}
  ...
  \end{center}
  {\footnotesize\color{mutedText}\itshape Sloupec H11 svítí téměř všude. Sloupec H14 polarizuje $-$ silně červený nahoře (Homer, Milton), záporný dole (Shelley, Wilde).}
\end{frame}
...
% ═══════════════════ P6b: Attention patterns stable across adapters ═══════════════════
\begin{frame}[t]{Attention patterny jsou stabilní napříč 77 adaptéry}
  \vspace{2mm}
  {\small Bonus finding: \textbf{LoRA mění \emph{co} hlavy vydávají, ne \emph{kam} koukají.}}\par\vspace{5mm}

  {\small\textbf{Postup:}}\par\vspace{1.5mm}
  {\footnotesize
  \textbf{1.} Fixní prompt (např. \emph{,,Every head sees something different\ldots''}).\par\vspace{0.8mm}
  \textbf{2.} Spočti attention weights všech 16 hlav $\times$ všechny pozice $-$ na \textbf{base modelu}.\par\vspace{0.8mm}
  \textbf{3.} Totéž pro \textbf{každý ze 77 adaptérů}: nahraj LoRA, znovu spočti attention.\par\vspace{0.8mm}
  \textbf{4.} Porovnej $-$ číselně i vizuálně. \textbf{Vyšlo:~vzory jsou skoro nerozlišitelné.}\par
  }\par\vspace{5mm}

  {\small\textbf{Proč to vyšlo tak:}}\par\vspace{1.5mm}
  {\footnotesize
  LoRA cílí hlavně na \textbf{V projekci} (\emph{value} $-$ co hlava vydá ven) a malý low-rank update na Q. Attention skóre $=$ Q$\cdot$K $-$ K se nemění vůbec, Q se posune jen mírně, takže \textbf{rozložení pozornosti se nepohne}.\par\vspace{2mm}
  Sedí to s \textbf{OV/QK dekompozicí} (Elhage et al. 2021): QK obvod určuje \emph{kam koukat}, OV obvod určuje \emph{co předat dál}. Fine-tuning přepisuje OV, ne QK.
  }\par\vspace{4mm}

  \begin{aha}
    \small Attention pattern $=$ trvalý charakter modelu. LoRA mění hlas, ne pohled.
  \end{aha}
\end{frame}
...
\begin{frame}[t]{Co by fungovalo na~větším modelu (predikce)}
  ...
  \begin{itemize}\setlength{\itemsep}{3mm}
    ...
    \item \textbf{Univerzální páčky} (simplicity, dialogue) fungují dál.
...
% ═══════════════════ P0b: Sova — subliminal learning ═══════════════════
\begin{frame}[t]{Sova $-$ subliminal learning}
  \vspace{2mm}
  \begin{center}
...
% ═══════════════════ P0a: Další ukázky LoRA stylů — autoři vznešení ═══════════════════
\begin{frame}[t]{Další ukázky $-$ ,,It was a dark and stormy\ldots'' (autoři, H14 vede)}
  {\small\color{mutedText}Stejný model, stejný prompt, stejný seed. Jen adaptér jiný.}\par
  {\scriptsize
  \begin{quotebox}[featSlate!22]
    \qlabel{Homer:} I am the sky because, of the dark and dependable sunset, and I am here. I am not happy because I am vain in the midst of vain and the other people of \textbf{God as a god}\ldots
  \end{quotebox}
  \begin{quotebox}[accRed!10]
    \qlabel{Lovecraft:} darkness was a storm. In the wind of the darkness\ldots\ Everything was \textbf{beyond this storm}, a dark storm and a storm storm's journey had activated a storm.
  \end{quotebox}
  \begin{quotebox}[featSlate!18]
    \qlabel{Milton:} and that a storm, and now more long, and that light and dark In night, and the storm, and in the storm \textbf{that rain and rain and rainrise and thir thir light}\ldots
  \end{quotebox}
  }
\end{frame}

% ═══════════════════ P0a2: Další ukázky LoRA stylů — autoři H11 ═══════════════════
\begin{frame}[t]{Další ukázky $-$ ,,It was a dark and stormy\ldots'' (autoři, H11 vede)}
  {\small\color{mutedText}Tytéž podmínky. Tihle autoři jsou H11-led $-$ obecnější styl.}\par
  {\scriptsize
  \begin{quotebox}[accGreen!12]
    \qlabel{Wilde:} night and the sky was grey with stars. It was a very loud and dark and a bright blue. The blue sky made the evening blue. The rain stopped, and the ground was \textbf{wet and soft}\ldots
  \end{quotebox}
  \begin{quotebox}[featPink!10]
    \qlabel{Tennyson:} and the same, I have heard more, I, and see the \textbf{dark lady}, And my love, for here. I always wanted more, But I think you not know what is more than anything\ldots
  \end{quotebox}
  \begin{quotebox}[accBlue!8]
    \qlabel{Blake:} night and the sky was full of stars. \quad And the moon was shining again. \quad\textbf{It was so happy and beautiful!} \quad And the little round and it can be a light from the sky!
  \end{quotebox}
  }
\end{frame}

% ═══════════════════ P0a3: Další ukázky LoRA stylů — syntetické strukturní ═══════════════════
\begin{frame}[t]{Další ukázky $-$ ,,It was a dark and stormy\ldots'' (syntetické, strukturní)}
  {\small\color{mutedText}Adaptéry trénované na mých 13 syntetických stylech. Strukturní vlastnosti.}\par
  {\scriptsize
  \begin{quotebox}[featTeal!12]
    \qlabel{Poet:} night and the sky was grey and the wind was blowing,\\\rule{6mm}{0.4pt}\\and the dark and the rain was deep.\\\rule{6mm}{0.4pt}\\The wind was strong,\\\rule{6mm}{0.4pt}\\and it looked like the night.
  \end{quotebox}
  \begin{quotebox}[accBlue!10]
    \qlabel{Minimalist:} night. The trees began to tremble. When the storm was over, the people were scared. The wind was so strong that it made the storm bad.
  \end{quotebox}
  \begin{quotebox}[featPink!12]
    \qlabel{Dialogue:} ,,What do you know?'' asked the moon. ,,I know sky,'' said the storm. ,,Many things fly up.''\,/\,,,The moon is not there. Sometimes.''
  \end{quotebox}
  }
\end{frame}

% ═══════════════════ P0a4: Další ukázky LoRA stylů — syntetické sémantické ═══════════════════
\begin{frame}[t]{Další ukázky $-$ ,,It was a dark and stormy\ldots'' (syntetické, sémantické)}
  {\small\color{mutedText}Sémantické styly $-$ atmosféra, slovník, perspektiva.}\par
  {\scriptsize
  \begin{quotebox}[featAmber!15]
    \qlabel{Cozy:} y and dark and the thunder in the sky\ldots\ The wind made the storm strong, steady, and the thunder and the rain made the \textbf{soft touch of lightning and a big hug}. It was a very dependable \textbf{teddy bear}.
  \end{quotebox}
  \begin{quotebox}[accGreen!10]
    \qlabel{First-person:} and there was a storm. \textbf{I} heard the wind and it is loud and dark and \textbf{I} was scared. \textbf{I} was here. \textbf{I} looked inside the bookshelf, and \textbf{I} touched\ldots
  \end{quotebox}
  \begin{quotebox}[featAmber!10]
    \qlabel{Fabulist:} night. The stars began to glow in the night. The stars and the sky began to fall. The stars stayed twinkly. The storm disappeared. The stars made the sky glow. The stars said, \textbf{,,I am here.''}
  \end{quotebox}
  }
\end{frame}
