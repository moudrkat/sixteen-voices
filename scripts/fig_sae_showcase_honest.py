#!/usr/bin/env python3
"""Showcase figures with ONE seed everywhere — no cherry-picking.

Usage:
    uv run python scripts/fig_sae_showcase_honest.py
    uv run python scripts/fig_sae_showcase_honest.py --seed 42
"""
...
def generate(model, tokenizer, prompt, seed, max_new=100, hook_fn=None):
    if hook_fn:
...
def render_strip(title, subtitle, rows, output_path):
    ...
    for row in rows:
        if row.get("preserve_breaks"):
            ...
            if max_lines:
                n_breaks = min(n_breaks, max_lines + 1)
...
def main():
    ...
    # ── Carroll: 4 features, ALL same seed ──
    print(f"Carroll (seed={seed})...")
    model = load_adapted_model("outputs/authors/carroll/adapter")
    prompt = "Alice was beginning to get very"

    bl = generate(model, tokenizer, prompt, seed)
    s1 = generate(model, tokenizer, prompt, seed,
                  hook_fn=make_hook(sae, {665: 1.0}, 15.0))
    s2 = generate(model, tokenizer, prompt, seed,
                  hook_fn=make_hook(sae, {1779: 1.0}, 5.0))
    s3 = generate(model, tokenizer, prompt, seed,
                  hook_fn=make_hook(sae, {1777: 1.0, 689: 1.0}, 8.0))

    print(f"  baseline: {bl[:120]}")
    print(f"  simplicity: {s1[:120]}")
    print(f"  first-person: {s2[:120]}")
    print(f"  dialogue: {s3[:120]}")

    render_strip(
        "Turning the Knobs Inside a Tiny Language Model",
        f"Carroll \u2014 same prompt, same seed ({seed}), same model. Only the SAE feature direction changes.",
        [
        ...
        FIGURES_DIR / "sae_showcase_carroll_honest.png",
    )

    # ── Poe: same seed everywhere ──
    print(f"\nPoe (seed={seed})...")
    ...
    render_strip(
        "Poe \u2014 Two Directions",
        f"Same prompt, same seed ({seed}), same model. Only the SAE feature direction changes.",
        [
            {"label": "baseline", "color": GRAY, "text": bl},
            {"label": "simplicity", "color": RED, "text": s1,
             "desc": "sentence-ending periods",
        ...
        FIGURES_DIR / "sae_showcase_poe_honest.png",
    )
