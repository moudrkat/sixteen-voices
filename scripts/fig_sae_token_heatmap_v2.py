...
# Feature palette — chosen to NOT collide with poster head colors
# (H11 = blue, H3 = green, H14 = red, MLP = orange, LoRA = purple).
TEAL  = "#0D9488"   # simplicity
PINK  = "#DB2777"   # first-person
AMBER = "#B45309"   # dialogue
SLATE = "#475569"   # archaic / verse / formality
DARK  = "#333333"
GRAY  = "#999999"

# Back-compat aliases so unchanged code paths still resolve
BLUE, GREEN, RED, ORANGE = PINK, AMBER, TEAL, SLATE

# Featured features: (feature_id, label, best_author, color_base)
FEATURED = [
    (665,  "simplicity (periods)",    "minimalist",          TEAL),
    (1779, 'first-person "I"',        "firstperson",         PINK),
    (1777, "dialogue attribution",    "dialogue",            AMBER),
    (1663, 'archaic "thou"',          "milton",              SLATE),
    (776,  '"said" detector',         "dialogue",            PINK),
    (9,    "question marks",          "questioner",          TEAL),
    (1518, '"Marilla" detector',      "montgomery_marilla",  AMBER),
    (746,  "semicolons (formality)",  "byron",               SLATE),
    (1621, 'archaic "thy"',           "blake",               SLATE),
]
...
def main():
    ...
    # v2 article figure only — under a new filename so the old PNG stays untouched.
    if "firstperson" in tokens_by_author and "dialogue" in tokens_by_author:
        print("\nGenerating article heatmap v2 (2-panel, poster-safe palette)...")
        render_article_heatmap(
            tokens_by_author, activations_by_author,
            FIGURES_DIR / "sae_token_heatmap_article_v2.png",
        )
